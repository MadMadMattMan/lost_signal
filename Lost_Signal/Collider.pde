class Collider {
  ...
  ArrayList<Integer> colliderLayers;

  // Box collider constructor
   Collider(PVector topLeft, PVector botRight, boolean isPhysicalCollider, ArrayList<Integer> colliderLayerRange) {
    // Sort and store x and y bounds
    xPoints = new PVector(min(topLeft.x, botRight.x), max(topLeft.x, botRight.x));
    yPoints = new PVector(min(topLeft.y, botRight.y), max(topLeft.y, botRight.y));

    this.isPhysicalCollider = isPhysicalCollider;
    this.colliderLayers = colliderLayerRange;
  ...
  public CollisionData isColliding(PVector otherPosition, float radius, ArrayList<Integer> layers) {
    // If a collision occurs
    if (isBetween(xPoints, otherPosition.x, radius) &&
        isBetween(yPoints, otherPosition.y, radius) &&
        layerMatch(layers)) {
        return new CollisionData(true, isPhysicalCollider, collisionNormal(otherPosition));// ------------ normal calculation
      }
  ...
  boolean layerMatch(ArrayList<Integer> otherLayers) {
    for (int i : colliderLayers) {
      if (otherLayers.contains(i))
        return true;
    }
    return false;
  }
  ...
  void calculateInterceptingIndexes() {
    ArrayList<Integer> calculatedIndecies = new ArrayList<>();
    ...
    for (int row = rowStart; row <= rowEnd; row++) {
      for (int col = colStart; col <= colEnd; col++) {
        ...
        if (index != null && !calculatedIndecies.contains(index)) {
          calculatedIndecies.add(index);
        }
      }
    }
  
    colliderIndecies = calculatedIndecies;
  }
...
public static class CollisionData{
  ...
  public PVector collisionNormal = new PVector();
   ...
   // Constructor
   CollisionData(boolean collisionResult, boolean physicalCollision, PVector collisionNormal) {
